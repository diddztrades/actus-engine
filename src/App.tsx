...
export default function App() {
  ...
  useEffect(() => {
    let cancelled = false;
    const liveAssetKey = actusModeLiveDisplayAsset ? `${actusModeLiveDisplayAsset.symbol}:${actusModeLiveDisplayAsset.timeframe}` : null;
  ...
  }, [actusModeLiveDisplayAsset?.symbol, actusModeLiveDisplayAsset?.timeframe]);
  ...
  const toggleBrowserAlerts = async () => {
    ...
    setProductPrefs((current) => ({
      ...current,
      browserAlertsEnabled: nextPermission === "granted",
    }));
  };
                ...
                <div style={{ display: "grid", gridTemplateColumns: "minmax(220px, 0.8fr) minmax(0, 1.2fr)", gap: 14 }}>
                  <div style={{ display: "grid", gap: 10 }}>
                    <div style={{ padding: "12px 14px", borderRadius: 14, background: "rgba(255,255,255,0.018)", border: "1px solid rgba(132,151,186,0.1)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        <div>
                          <div style={{ fontSize: 11, color: "#7f8da8", letterSpacing: "0.1em", textTransform: "uppercase" }}>Browser notifications</div>
                          <div style={{ marginTop: 6, fontSize: 13, color: "#d7e1f4", lineHeight: 1.45 }}>
                            External alerts fire from existing ACTUS transitions only when the app is in the background.
                          </div>
                        </div>
                        {badge(browserAlertStatusLabel, "#d7e1f4", "rgba(255,255,255,0.03)", "rgba(142,160,191,0.14)")}
                      </div>
                      <div style={{ marginTop: 10, display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                        <div style={{ fontSize: 12, color: "#8ea0bf" }}>
                          {browserNotificationPermission === "denied"
                            ? "Browser permission is blocked. Re-enable notifications in the browser to turn this back on."
                            : browserNotificationPermission === "unsupported"
                              ? "This browser does not support system notifications."
                              : productPrefs.browserAlertsEnabled
                                ? "ACTUS will deliver external alerts for throttled in-app and transition events."
                                : "Enable once to let ACTUS push alerts off-screen."}
                        </div>
                        <button
                          type="button"
                        ...
                        >
                          {productPrefs.browserAlertsEnabled ? "Disable" : "Enable"}
                        </button>
                      </div>
                    </div>
                    <div style={{ padding: "12px 14px", borderRadius: 14, background: "rgba(255,255,255,0.018)", border: "1px solid rgba(132,151,186,0.1)" }}>
